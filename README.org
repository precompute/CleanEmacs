...
- Commit :: =91c9e9883488d715a30877dfd7641ef4b3c62658=
...
#+BEGIN_SRC
'CPPFLAGS=-I/opt/customlibs/mps_artifacts -I/opt/customlibs/tree-sitter/include'
'LDFLAGS=-L/opt/customlibs/mps_artifacts -L/opt/customlibs/tree-sitter/lib -Wl,-rpath,/opt/customlibs/tree-sitter/lib'
'CFLAGS=-O4 -flto -fuse-linker-plugin -march=native -mtune=native -fomit-frame-pointer'
'CXXFLAGS=-O4 -flto -fuse-linker-plugin -march=native -mtune=native -fomit-frame-pointer'
...
** Tree-Sitter
When compiled with =--with-tree-sitter=, Emacs requires per-language grammar binaries for tree-sitter functionality.  These go in =user-emacs-directory/tree-sitter=.  You can compile the ones you want manually, or pick and choose from the excellent [[https://github.com/emacs-tree-sitter/tree-sitter-langs][tree-sitter-langs]] repo.
The =.so= files need to  be prepended with the string =libtree-sitter-=.
